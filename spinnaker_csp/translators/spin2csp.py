...
class spin2csp:
    ...
    def __init__(self, length, dimensions=3):
        ...
        if dimensions==3:
            for x in range(length):
                for y in range(length):
                    for z in range(length):
                        if x != length-1: constraints.append((lattice[x][y][z], lattice[x+1][y][z]))
                        if y != length-1: constraints.append((lattice[x][y][z], lattice[x][y+1][z]))
                        if z != length-1: constraints.append((lattice[x][y][z], lattice[x][y][z+1]))
                        if x >= 0: constraints.append((lattice[x][y][z], lattice[x-1][y][z]))
                        if y >= 0: constraints.append((lattice[x][y][z], lattice[x][y-1][z]))
                        if z >= 0: constraints.append((lattice[x][y][z], lattice[x][y][z-1]))
            ...
            for x in range(length):
                for y in range(length):
                    if x != length - 1: constraints.append((lattice[x][y], lattice[x + 1][y]))
                    if x >= 0: constraints.append((lattice[x][y], lattice[x - 1][y]))
                    if y != length - 1: constraints.append((lattice[x][y], lattice[x][y + 1]))
                    if y >= 0: constraints.append((lattice[x][y], lattice[x][y - 1]))
                # Create periodic condition on boundaries:
                constraints.append((lattice[x][length - 1], lattice[x][0]))
                constraints.append((lattice[length - 1][x], lattice[0][x]))
        else: # dimensions == 1:
            for x in range(length):
                if x != length - 1: constraints.append((lattice[x], lattice[x + 1]))
                if x >= 0: constraints.append((lattice[x], lattice[x - 1]))
            # Create periodic condition on boundaries:
            constraints.append((lattice[length-1],lattice[0]))
        formatted_constraints = []
        for constraint in constraints:
            formatted_constraints.append({'source': constraint[0], 'target': constraint[1]})
