...
parser.add_argument(
    "lattice",
    help="name of lattice from: AF3D, FM3D, SG50, SG10, ring",
    type=str,
    default="ring",
)
parser.add_argument(
    "-n", "--name", help="name to use for naming files", type=str, default=None
)
parser.add_argument(
    "-l",
    "--length",
    help="number of spins on each side of the lattice",
    type=int,
    default=10,
)
...
systems = {
    "AF3D": [3, False, "inhibitory", 1, "AF3D"],  # Antiferromagnetic 3D lattice.
    ...
    "SG50": [
        ...
        0.5,
        "SGI",
    ],  # 2D spin glass, 50% probability of antiferromagnetic coupling.
    "SG10": [
        2,
        True,
        "inhibitory",
        0.1,
        "SGII",
    ],  # 2D spin glass, 10% probability of antiferromagnetic coupling.
    "ring": [1, False, "inhibitory", 1, "chain"],
}  # 1D spin system with connected ends.
...
spin = CSP(
    variables_number=crystal.var_num,
    domain_size=crystal.dom_num,
...
spin.build_stimulation_pops(
    n_populations=1, shrink=1.0, stim_ratio=1.0, full=False, phase=0.0
)  # shrink = 1.0
